...
This repository is scoped to public/open-source caller repositories, signing through an independent Google Cloud project and KMS keyring.
...
2. KMS key ring with signing keys (EC P-384 or RSA 4096)
3. Workload Identity Federation pool/provider trusting the caller repo's GitHub OIDC token
4. Service account with `roles/cloudkms.signer` on the relevant keys
5. Code-signing certificate (PEM)

The WIF attribute condition must allow the **caller** repository. For example, to allow `Dynatrace/dynatrace-otel-collector`:

```
attribute.repository == "Dynatrace/dynatrace-otel-collector"
```
...
| Caller-repository allowlist | `ALLOWED_CALLER_REPOSITORIES` env (see [below](#caller-repository-allowlist)) | `ALLOWED_CALLER_REPOSITORIES` env (see [below](#caller-repository-allowlist)) |

### Caller-repository allowlist

Both reusable workflows check `github.repository` against a hardcoded allowlist as the **first step** of the signing job, failing fast with a clear message if the caller isn't listed.

This is a convenience check, not the real security boundary — the authoritative access control is the GCP Workload Identity Federation `attribute.repository` condition. The allowlist exists so a misconfigured caller gets a clear failure message instead of a confusing GCP auth error; don't rely on it as the actual security gate.

To authorize a new caller repo, edit the `ALLOWED_CALLER_REPOSITORIES` env block at the top of `.github/workflows/sign-linux.yml` and/or `.github/workflows/sign-windows.yml`. One repository per line, in `owner/repo` form. Keep this list in sync with the WIF attribute condition on the GCP side.
...
- This repository is scoped exclusively to public/open-source signing; Dynatrace-internal callers use a separate, non-public copy of these workflows
